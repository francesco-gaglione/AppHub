<script lang="ts">
    ...
    import {
        installedAppsList,
        uninstallApp,
    } from "$lib/helpers/tauriCommands/appImageCommands";
    ...
    const debouncedFilterApps = debounce((search: string) => {
        filterApps(search);
    }, 400);
    ...
    const filterApps = (search: string) => {
        ...
        if (search.length > 0) {
            filteredAppList.apps = appList.apps.filter((app) =>
...
<div
    class="flex flex-col bg-base-200 rounded-box mx-10 mt-10 p-5"
...
>
    <div class="mb-5 flex flex-row justify-between items-center">
        ...
        <input
            on:input={(e) => {
                debouncedFilterApps(e.target.value);
            }}
            type="text"
...
>
    {#if appDetails}
        <div class="flex flex-col">
            ...
            <p>
                {$t("applist.app_category")}: {appDetails.category
                    ? appDetails.category
                    : ""}
            </p>
        </div>
